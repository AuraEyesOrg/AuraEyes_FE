...
test.describe('Flow 04 - Organisation Slot Booking and Offline Appointment', () => {
  test('patient reviews organisation slots and completes offline appointment booking from clinics page', async ({ browser }) => {
    ...
    await expect(page.getByRole('heading', { name: /Organisation Slots/i })).toBeVisible({ timeout: 10_000 });
    await expect(page.getByRole('link', { name: /Book More Slot/i }).first()).toBeVisible({ timeout: 10_000 });

    await page.getByRole('link', { name: /Book More Slot/i }).first().click();
    await expect(page).toHaveURL(/\/patient\/clinics/);

    const searchInput = page.getByPlaceholder(/Search organisation by name, city, or address/i);
    await searchInput.fill(OrgName);

    await page.locator('button', { hasText: new RegExp(OrgName, 'i') }).first().click();

    await page.locator('input[type="date"]').fill(slotDate);

    const reasonInput = page.getByPlaceholder(/Blurred vision, routine follow-up/i);
    ...
    await expect(page.getByText(/Đặt lịch thành công|Appointments/i)).toBeVisible({ timeout: 20_000 });

    await page.goto('/patient/appointments');
    await expect(page.getByRole('heading', { name: /Organisation Slots/i })).toBeVisible();
    await expect(page.getByText(new RegExp(OrgName, 'i')).first()).toBeVisible({ timeout: 15_000 });
