...
test.describe('Flow 05 - AI Quota Purchase for Screening Service', () => {
  test('patient purchases additional ai quota from screening service and quota is updated', async ({ browser }) => {
    ...
    await query(`UPDATE "Wallets" SET "Balance" = 800000 WHERE "UserId" = $1`, [PatientUserId]);
    ...
    await expect(page.getByRole('button', { name: /Start AI Analysis/i })).toBeVisible({ timeout: 15_000 });
    ...
    await expect(page.getByText(/Out of quota|Bạn đã dùng hết lượt AI/i)).toBeVisible({ timeout: 20_000 });

    const quotaPurchaseBtn = page.locator('button', { hasText: /Mua|Buy|Quota|lượt/i }).first();
    if ((await quotaPurchaseBtn.count()) > 0) {
      await quotaPurchaseBtn.click();
    } else {
      await page.goto('/patient/screening/new');
      await page.locator('button', { hasText: /Mua|Buy|Quota|lượt/i }).first().click();
    }

    await expect(page.locator('[role="dialog"]')).toBeVisible({ timeout: 15_000 });

    const packageBtn = page.locator('button, div[role="button"]', { hasText: /\d+\s*lượt|\d+/i }).first();
    await packageBtn.click();

    await page.locator('button', { hasText: /Confirm|Pay|Purchase|Mua/i }).first().click();
    await expect(page.getByText(/success|thành công|purchased/i)).toBeVisible({ timeout: 20_000 });

    const quotaAfterBuy = await query<{ UsedAiQuota: number; PurchasedAiQuota: number }>(
