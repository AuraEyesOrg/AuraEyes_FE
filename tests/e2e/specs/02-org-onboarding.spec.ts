...
const SYSTEM_ADMIN_EMAIL = process.env.E2E_ROLE_EMAIL_SYSTEM_ADMIN ?? 'systemadmin@gmail.com';
...
test.describe('Flow 02 - Organization Onboarding & Contract Approval', () => {
  test('admin approves pending org request, org uploads contract, admin verifies, org reaches dashboard', async ({
    ...
    await loginByUi(pageAdmin, SYSTEM_ADMIN_EMAIL, DEFAULT_PASSWORD, /\/system-admin/);

    // Navigate to organisations approval page
    await pageAdmin.goto('/system-admin/organisations');
    await expect(pageAdmin.getByRole('heading', { name: /Organisation|Onboarding/i })).toBeVisible({
      timeout: 10_000,
    });

    const approveBtn = pageAdmin
      .getByRole('button', { name: /Xác nhận\s*&\s*cấp tài khoản|Approve|cap tai khoan/i })
      .first();
    ...
    const approvalResponsePromise = pageAdmin.waitForResponse(
      (response) =>
        ...
        response.url().includes('/system-admin/organisations/onboarding-requests/') &&
    ...
    await expect(pageAdmin.getByText(/Đã cấp tài khoản thành công|approved|success/i)).toBeVisible({
      timeout: 15_000,
    });

    await ctxAdmin.close();

    const provisionedOrgEmail = approvalResult?.orgAdminEmail ?? ORG_ADMIN_EMAIL_FALLBACK;
    const provisionedOrgPassword = approvalResult?.temporaryPassword ?? DEFAULT_PASSWORD;
    ...
    await loginByUi(pageOrg, provisionedOrgEmail, provisionedOrgPassword, /\/organisation/);

    // Should land on contract page
    await expect(pageOrg).toHaveURL(/\/organisation\/contract/, { timeout: 10_000 });
    ...
    await expect(pageOrg.getByText(/Đã upload|Đang chờ admin xác nhận|pending/i)).toBeVisible({
      timeout: 15_000,
    });
    ...
    await loginByUi(pageAdmin2, SYSTEM_ADMIN_EMAIL, DEFAULT_PASSWORD, /\/system-admin/);

    // Navigate to contracts verification page
    await pageAdmin2.goto('/system-admin/contracts');
    await expect(pageAdmin2.getByRole('heading', { name: /Contract|Verification/i })).toBeVisible({
      timeout: 10_000,
    });
    ...
    await expect(pageAdmin2.getByText(/Hiệu lực|Active|verified|success/i).first()).toBeVisible({
      timeout: 15_000,
    });
    ...
    await loginByUi(pageOrg2, provisionedOrgEmail, provisionedOrgPassword, /\/organisation/);

    // Contract gate should be cleared.
    await expect(pageOrg2).not.toHaveURL(/\/organisation\/contract/);
    await expect(pageOrg2).toHaveURL(/\/organisation\/(dashboard|slot-management|appointments|doctors)/);
