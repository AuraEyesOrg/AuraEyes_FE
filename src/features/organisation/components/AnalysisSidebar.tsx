...
const AnalysisSidebar: React.FC<AnalysisSidebarProps> = ({
  ...
  return (
    <aside className="w-80 flex-none flex flex-col bg-[#182626] border-l border-[#283939] overflow-hidden z-20 shadow-xl">
      {/* Risk Summary Card */}
      <div className="p-5 border-b border-[#283939] bg-gradient-to-b from-[#182626] to-[#131f1f]">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-white text-sm font-semibold uppercase tracking-wider">
            AURA Risk Score
          </h3>
          <div className="group relative cursor-help">
            <Info className="w-4 h-4 text-[#9db9b9]" />
            <div className="absolute right-0 top-6 w-48 p-2 bg-black border border-[#283939] text-xs text-[#9db9b9] rounded hidden group-hover:block z-50 shadow-xl">
        ...
        <div className="flex items-end gap-3 mb-3">
          ...
          <span className="text-sm text-[#9db9b9] mb-1.5">/ 10</span>
          {Number(riskScore) > 5 && (
        ...
        <div className="w-full bg-[#283939] rounded-full h-2 overflow-hidden">
          <div
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6">
        {/* Analysis Trigger */}
        <div className="bg-[#283939]/20 rounded-lg p-1 border border-[#283939] flex flex-col gap-2">
          {!analyzed && !isAnalyzing ? (
            <div className="p-3 text-center">
              <p className="text-[#9db9b9] text-xs mb-3">
            ...
            <div className="p-4 flex flex-col items-center justify-center gap-3">
              <div className="size-8 border-4 border-[#283939] border-t-[#13ecec] rounded-full animate-spin"></div>
            ...
            <div className="flex flex-col gap-2 px-3 py-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={onAnalyze}
                  className="text-xs text-[#9db9b9] hover:text-white underline"
                >
                  Re-run
                </button>
              </div>
        ...
        <div className="space-y-3">
          <h4 className="text-xs font-semibold text-[#9db9b9] uppercase tracking-wider mb-2">
            Annotation Layers
          </h4>
        ...
        <div className="h-px bg-[#283939] w-full"></div>
        ...
        <div className="space-y-3">
          ...
          {anomalies.length === 0 && analyzed && (
            <div className="text-sm text-[#9db9b9] italic text-center py-4">
          ...
          {anomalies.map((anomaly) => (
            <div
              key={anomaly.id}
              className={`bg-[#283939]/30 rounded-lg p-3 border border-[#283939] hover:border-[#13ecec]/50 transition-colors cursor-pointer group ${anomaly.confidence < 50 ? 'opacity-70' : ''}`}
            >
              <div className="flex justify-between items-start mb-1">
                ...
                <span
                  className={`text-xs font-mono ${anomaly.confidence < 50 ? 'text-[#9db9b9]' : 'text-[#13ecec]'}`}
                >
                  {anomaly.confidence}%
                </span>
              </div>
              {anomaly.description && (
                <p className="text-xs text-[#9db9b9] mb-2">
                  {anomaly.description}
                </p>
              )}
      ...
      {/* Footer Actions */}
      <div className="p-4 border-t border-[#283939] bg-[#182626] space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-[#283939] bg-[#283939]/50 text-white text-sm font-bold hover:bg-[#283939] transition-colors">
...
const ToggleItem: React.FC<ToggleItemProps> = ({
  ...
  return (
    <label className="flex items-center justify-between cursor-pointer group select-none py-1">
      <span className="text-sm text-white group-hover:text-[#13ecec] transition-colors">
        {label}
      </span>
      <button
        type="button"
