...
import {
  adminEmploymentTypeChangeRequestsApi,
  type AdminEmploymentTypeChangeRequestItem,
...
export default function SystemAdminEmploymentTypeChangeRequestsPage() {
  ...
  const listQuery = useQuery({
    ...
    queryFn: () =>
      adminEmploymentTypeChangeRequestsApi.getRequests(
        pageNumber,
        pageSize,
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['system-admin', 'employment-type-change-requests'],
      });
    },
    onError: (error) => {
      toast.error(
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                            {item.currentEmploymentType} {'->'}{' '}
                            {item.targetEmploymentType}
                          </p>
